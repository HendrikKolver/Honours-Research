...
\hspace{0pt} \\
Most self-recovering techniques have a tradeoff trade-off concerning the quality of the image after embedding and the quality of the image after recovery.
...
\subsection{Introduction}
Self-recovering images are images that have their own image content embedded into themselves using image steganography techniques. //TODO <Reference> 
...
//TODO <Get better definition and reference> Steganography is the process of hiding information in such a way that it does not get detected \cite{johnson1998exploring}.
...
\begin{itemize}
	\item Invisibility: How easy it is to visually detect that information has been embedded using steganography. //TODO <reference>
	\item Robustness: How much of the image that is embedded into can be modified without compromising the embedded information. //TODO <definition and reference based on traditional stego concepts>
...
LSB is a very simple steganography algorithm and many methods exist for detecting the hidden information //TODO <Reference>

\hspace{0pt} \\
Bit plane complexity steganography (BPCS) is another steganographic embedding method that was developed in <year> by <authors> \cite{beaullieubpcs}. 
...
\hspace{0pt} \\
The BPCS algorithm is a high capacity embedding algorithm. //TODO <reference>
...
Self embedding is a scheme where image content is embedded into the image itself //TODO <reference. The embedded content can then later be used to recover damaged or modified parts of the image without accessing the original image itself \cite{fridrich1999images}.
...
\begin{itemize}
	\item Peak signal to noise ratio (PSNR): PSNR is an expression for the ratio between the maximum possible value (power) of a signal and the power of distorting noise that affects the quality of its representation. //TODO <reference> PSNR is represented as a logarithmic decibel scale. In terms of images PSNR can objectively measure the amount of noised introduced into an image when compared with the original image. 
